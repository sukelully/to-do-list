...
import { openModal, getRequiredElement } from '../utils';
import { Project } from '../models';

const projectMenu = getRequiredElement<HTMLUListElement>('project-dropdown');
const projectSelect = getRequiredElement<HTMLSelectElement>('project');

const initAddTaskModal = () => {
  document.addEventListener('DOMContentLoaded', () => {
    // Priority dropdown elements
    const priorityBtn = getRequiredElement<HTMLButtonElement>(
      'priority-dropdown-btn'
    );
    const priorityMenu =
      getRequiredElement<HTMLUListElement>('priority-dropdown');
    const prioritySelect = getRequiredElement<HTMLSelectElement>('priority');

    // Project dropdown elements
    const projectBtn = getRequiredElement<HTMLButtonElement>(
      'project-dropdown-btn'
    );

    const cancelBtn = getRequiredElement<HTMLButtonElement>('cancel-btn');
    const addTaskModal = getRequiredElement<HTMLDivElement>('add-task-modal');
...
const populateProjects = (projectsArray: Project[]) => {
  const projectDropdownBtn = getRequiredElement<HTMLButtonElement>(
    'project-dropdown-btn'
  );
  ...
  projectsArray.forEach((project, index) => {
    ...
    li.className =
      'flex items-center gap-2 px-4 py-2 cursor-pointer hover:bg-stone-200';
